<template>
    ...
    >
      ...
      <el-form-item label="新闻类型" prop="type">
        <el-select
          ...
          placeholder="请选择新闻类型"
        >
          <el-option label="图片新闻" :value="1" />
          <el-option label="文字新闻" :value="2" />
        </el-select>
      </el-form-item>
  ...
  <ContentWrap>
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="新闻标题" align="center" prop="title" min-width="200" show-overflow-tooltip />
      <el-table-column label="新闻类型" align="center" prop="type">
        <template #default="scope">
          <el-tag :type="scope.row.type === 1 ? 'primary' : 'success'">
            {{ scope.row.type === 1 ? '图片新闻' : '文字新闻' }}
          </el-tag>
        </template>
      </el-table-column>
...
const handlePublish = async (id: number) => {
  try {
    // 发布的二次确认
    await message.confirm('是否确认发布该新闻？')
    // 发起发布
...
const handleUnpublish = async (id: number) => {
  try {
    // 下架的二次确认
    await message.confirm('是否确认下架该新闻？')
    // 发起下架
